...
use App\Industry;
use App\Mail\DeleteFrontJobMail;
use App\Mail\JobApprovalMail;
use App\Mail\JobRejectionMail;
...
use App\Models\Admin\AdminInterviewStage;
use App\Models\Admin\AdminInterviewStatus;
...
use App\Models\Admin\AdminTodo;
...
class AdminController extends Controller
{
    ...
    {
        return json_encode(AdminPrivileges::all());
    }
    ...
    {
        $exist = AdminPrivileges::where('name', $request->privilegeName)->exists();
//            ->orWhere('color' , $request->statusColor)->exists() ;

        if ($exist) {
            return json_encode($exist);
        } else {
            $privilege = new AdminPrivileges();
    ...
    {
        AdminPrivileges::where('id', $request->privilegeId)->delete();
    }

    public function editprivilege(Request $request)
    {

        $privilege = AdminPrivileges::find($request->id);

        $statusExist = AdminPrivileges::where('name', $request->eLetestName)
...
//    Sub Admin Operations
    ...
    public function jobStore(Request $request)
    {
        ...
        $job->job_discription = $description;
...
//        return json_encode($arrayName);
    ...
    public function candidateStore(Request $request)
    {
        ...
        $candidate->admin_status_id = $request->status_id;
    ...
    public function ajaxCandidateAll(Request $request)
    {
        ...
        if (!empty($request->status) || $request->status != 0) {
            ...
            if ($status == True) {
                $where .= ',' . ['state', 'like', "%" . $request->state . "%"];
                $where .= " AND ( admin_status_id =" . $request->status . ") ";
            } else {
                $where .= "  admin_status_id =" . $request->status;
            }
            $status = True;
        }
        ...
        $arrayName = array('0' => $canidateCountWhere, '1' => $candidates);
        return json_encode($arrayName);
    }
    ...
    public function candidateOtm()
    {
        ...
        return view('admin.candidate.otm')
            ->with(['statuses' => AdminStatus::orderby('name', 'asc')->get(),
                'pipelineClients' => AdminClient::all(), 'stsWithoutOtm' => $stsWithoutOtm]);
    }
...
//        return true;
    ...
    {
        $candidate = AdminCandidate::with(['adminStatus', 'adminNotes', 'adminResumes', 'adminEducation'])
            ->where(['id' => $candidateId])->get();
    ...
    {
        $statusId = AdminCandidate::where('id', $request->candId)->pluck('admin_status_id');
        if ($request->status == $statusId[0]) {
            $sameValBool = true;
            return json_encode($sameValBool);
        } else {
            AdminCandidate::where('id', $request->candId)
                ->update(['admin_status_id' => $request->status]);
        }
    ...
    public function updatepipelineStatusAjax(Request $request)
    {
        ...
        $statusId = AdminCandidate::where('id', $request->candId)
            ->pluck('admin_status_id');
        if ($request->status == $statusId[0]) {
            $sameValBool = true;
            return json_encode($sameValBool);
        } else {
        ...
        if ($user_id != 1) {
            $user = AdminUser::find(1);
            $candidate = AdminCandidate::find($request->candId);
            $details =
                [
    ...
    {
        $this->authorize('delete', AdminCandidate::class);
        $candidate = AdminCandidate::find($request->candId);
        $resumes = $candidate->adminResumes;
        foreach ($resumes as $resume) {
            if (file_exists(public_path() . '/files/' . $resume->resume)) {
                unlink(public_path() . '/files/' . $resume->resume);
            }
            $resume->delete();
        }
        $job = AdminJob::find($request->candId);
        if ($job) {
            $job->admin_candidate_id = null;
            $job->save();
        }
    ...
    public function candidateEdit($candId)
    {
        if (url()->current() != url()->previous()) {
            session(['previous_url' => url()->previous()]);
        }
        $candidate = AdminCandidate::with('adminResumes')->where('id', $candId)->get();
        return view('admin.candidate.edit')->with(['candidate' => $candidate,
            'statuses' => AdminStatus::orderby('name', 'asc')->get(), 'education' => AdminEducation::all()]);
    }

    public function candidateUpdate(Request $request)
    {
        $candidate = AdminCandidate::find($request->id);
...
//        dd($request->previous_url);
    ...
    {
        AdminCandidate::whereIn('id', $request->candIds)
            ->update(['admin_status_id' => $request->status]);
    ...
    {
//        dd($request->all());
        $todo = AdminTodo::find($request->id);
        $todo->action_id = $request->action;
        $todo->time = $request->time;
        $date = date('Y-m-d', strtotime(str_replace('-', '/', $request->date)));
        $todo->date = $date;
...
//        dd($request->all());
//        AdminTodo::destroy($request->id);
        AdminTodo::whereIn('id', $request->list)->delete();
    ...
    {
        $todo = AdminTodo::with('action')->where('user_id', Auth::id())->get();
    ...
    public function todolist(Request $request)
    {
        ...
        {
            $todo = AdminTodo::with('action')->where('user_id', Auth::id())->offset($offset)->limit(10)->get();
            $total = AdminTodo::count();
    ...
    public function createdAction(Request $request)
    {
        ...
        $exist = AdminTodoAction::where('name', $request->Name)->exists();
//            ->orWhere('color' , $request->statusColor)->exists() ;
...
//dd($request->all());
        ...
        $action = AdminTodoAction::find($request->id);

        $actionExist = AdminTodoAction::where('name', $request->Text)
            ->Where('id', '!=', $request->id)->exists();
...
//
    ...
    {
        $todo = AdminTodo::where('action_id', $request->actionId)->count();
//        dd($todo);
//
//        //it means it has a candidate so it cant be delete.
        if ($todo > 0) {
            ...
            AdminTodoAction::where('id', $request->actionId)->delete();
        }
    }
    ...
    {
        $this->authorize('delete', AdminContract::class);
        AdminContract::destroy($request->interview_id);
    }

    public function signContract(Request $request)
    {
        $contract = AdminContract::find($request->contract_id);
        $contract->status = 1;
        $contract->save();
    }
...
//            'edit_job_title'=> 'required',
        ...
        $contract = AdminContract::find($request->contract_id_edit);
...
//        $contract->email = $request->edit_email_address;
        ...
        if ($request->hasfile('contract_file')) {
            $date = date('Y_m_d-H-i-s');
            $fileName = $date . '.' . $request->file('contract_file')->getClientOriginalName();
            $request->file('contract_file')->move(public_path('/files/'), $fileName);
...
//        ]);
        ...
        $contract->admin_client_id = $request->client_id;
...
//        $client = Client::with('contracts')->where('id',$request->client_id)->get();
        $client_contract = AdminContract::with('adminClient')
...
//        dd($client_contract[0]['client']['email']);
        $email = $client_contract[0]['adminClient']['email'];
//        dd($email);
...
//    dd($request->emails);
    ...
    {
        AdminPlacement::destroy($request->id);
    }

    public function placementCreate(Request $request)
    {
        $placement = new AdminPlacement();
        $placement->admin_candidate_id = $request->candidate;
        $placement->admin_job_id = $request->job;
        $placement->admin_client_id = $request->company;
        ...
        $placement->save();
        $status = AdminStatus::where('name', 'HIRED')->first();
        $candidate = AdminCandidate::find($placement->admin_candidate_id);
        $candidate->admin_status_id = $status->id;
        $candidate->save();
...
//            'fee'=> 'required',
//        ]);
        $placement = AdminPlacement::find($request->id);
        $placement->admin_candidate_id = $request->candidate;
        $placement->admin_job_id = $request->job;
        $placement->admin_client_id = $request->company;
        $placement->salary = $request->salary;
    ...
    {
        $this->authorize('view', AdminContract::class);
...
//        $clients = Client::has('contracts')->get();
        return view('admin.placement.placement')
            ->with(['pipelineClients' => $pipelineClients, 'jobCandidates' => $candidates, 'employees' => $employees]);

    }

    public function searchPlacement(Request $request)
    {
        $placements = AdminPlacement::with(['adminJob', 'adminCandidate', 'adminClient'])
            ->where('id', $request->id)
            ->get();
    ...
    public function listPlacement(Request $request)
    {
        ...
        if (!empty($request['job_name'])) {
            ...
            array_push($where, ['admin_placements.admin_job_id', $j_name]);
            array_push($where1, ['job_id', $j_name]);
        }
        if (!empty($request['company_name'])) {
            $c_name = $request['company_name'];
            array_push($where, ['admin_placements.admin_client_id', $c_name]);
            array_push($where1, ['admin_client_id', $c_name]);
        }
        ...
        if (!empty($request['candidate_name'])) {
            ...
            array_push($where, ['admin_placements.admin_candidate_id', $cand_name]);
            array_push($where1, ['admin_candidate_id', $cand_name]);
        }

        if (Auth::user()->can('viewAny', AdminPlacement::class)) {
            $totaldata = AdminPlacement::with(['adminJob', 'adminCandidate', 'adminClient', 'recruiter'])
                ->where($where1)->count();

            $placements = AdminPlacement::join('admin_jobs', 'admin_jobs.id', '=', 'admin_placements.admin_job_id')
                ->join('admin_candidates', 'admin_candidates.id', '=', 'admin_placements.admin_candidate_id')
                ->join('admin_clients', 'admin_clients.id', '=', 'admin_placements.admin_client_id')
                ->join('admin_users', 'admin_users.id', '=', 'admin_placements.recruiter_id')
                ->select('admin_placements.*', 'admin_clients.company_name', 'admin_jobs.jobtitle',
                    'admin_candidates.name', 'admin_candidates.id as candidate_id', 'admin_users.name as recruiter_name')
                ->where($where)
                ->offset($offset)
                ->limit($limit)
                ->orderBy('admin_clients.company_name', $order)
                ->get();
        } else {
            $totaldata = AdminPlacement::with(['adminJob', 'adminCandidate', 'adminClient', 'recruiter'])
                ->where($where1)
        ...
        foreach ($placements as $placement) {
            ...
            $columns[] = array(
                ...
                'candidate' => "<a href=" . url('admin/candidate/detail/') . "/" .
                    $placement['candidate_id'] . ">" . $placement['name'] . "</a>",
                'start_date' => $s_date,
    ...
    public function contractDetails(Request $request)
    {
        ...
        $contract = AdminClient::with(['adminContracts' => function ($query) use ($offset) {
            $query->offset($offset)->limit(1);
        }])->where('id', $request->client_id)->get();
        $total = AdminClient::withCount('adminContracts')
            ->where('id', $request->client_id)
            ->pluck('admin_contracts_count')->first();
    ...
    public function contractsAjax(Request $request)
    {
        ...
        if ($request->client_id != 0) {
            $clients = AdminClient::has('adminContracts')
                ->with('adminNotes', 'adminContracts')
                ->where('id', $request->client_id)
                ->offset($offset)
                ->limit(10)
                ->get();
            $total = AdminClient::has('adminContracts')
                ->where('id', $request->client_id)
                ->count();
        } else {
            $clients = AdminClient::has('adminContracts')
                ...
                ->get();
            $total = AdminClient::has('adminContracts')->count();
        }
        $drop_down = AdminClient::has('adminContracts')->get();
    ...
    public function scheduledInterviews()
    {
        ...
        $where = DB::table('admin_client_users')
            ->where('admin_user_id', $user_id)
            ->pluck('admin_client_id');
        $Clients = AdminClient::whereIn('id', $where)->get();
        $Candidates = AdminCandidate::all();
        $employees = AdminUser::where([['is_super_admin', 0], ['id', '!=', $user_id]])->get();
        return view('admin.interview.scheduled-interviews')
            ->with(['clients' => $Clients, 'candidates' => $Candidates, 'employees' => $employees]);
    }

    public function searchInterviews(Request $request)
    {
        $interview = AdminScheduleInterview::with('adminJobs', 'adminCandidates', 'adminClients', 'adminStatus', 'adminReceiverEmail')
            ->find($request->id);
...
//        return json_encode($interview);
    ...
    public function updateScheduledInterviewStatus(Request $request)
    {
        $interview = AdminScheduleInterview::with(['adminJobs', 'adminCandidates', 'adminClients', 'adminStatus', 'adminReceiverEmail'])
            ->find($request->id);
        $interview['admin_status_id'] = $request->status_id;
        $interview->save();
    ...
    public function scheduledinterviewslists(Request $request)
    {
        ...
        $status = AdminStatus::where([['name', '=', 'DECLINED']])->orwhere([['name', '=', 'REJECTED']])->pluck('id');

        if ($request['type'] == 'Rejected') {
            $reject = 1;
            foreach ($status as $sta) {
                if ($ck == 0) {
                    $orwhere1 .= ' admin_status_id = ' . $sta;
                    $ck = 1;
                } else {
                    $orwhere1 .= ' OR admin_status_id = ' . $sta;
                }

            }
        } else {
            foreach ($status as $sta) {
                array_push($where, ['admin_status_id', '!=', $sta]);
            }
        }
        ...
        if (!empty($request['job_name'])) {
            $j_name = $request['job_name'];
            array_push($where, ['admin_job_id', $j_name]);
        }
        if (!empty($request['company_name'])) {
            $c_name = $request['company_name'];
            array_push($where, ['admin_client_id', $c_name]);
//            $s2=1;
        }
        if (!empty($request['candidate_name'])) {
            $cand_name = $request['candidate_name'];
            array_push($where, ['admin_candidate_id', $cand_name]);
        }
        array_push($where, ['admin_user_id', Auth::id()]);

        if ($reject == 0) {
            $totaldata = AdminScheduleInterview::with(['adminJobs', 'adminCandidates', 'adminClients', 'adminStatus', 'adminReceiverEmail'])
                ->where($where)
                ->count();
            $schedule_interviews = AdminScheduleInterview::with(['adminJobs', 'adminCandidates', 'adminClients', 'adminStatus', 'adminReceiverEmail'])
                ->where($where)
                ->offset($offset)
                ->limit($limit)
                ->get();
        ...
        foreach ($schedule_interviews as $schedule_interview) {
            ...
            $columns[] = array(
                'status_id' => $schedule_interview['adminStatus']['id'],
                'id' => $schedule_interview['id'],
                'company' => '<b style="color: #4d9a10">' . $schedule_interview['adminClients']['company_name'] . '</b>',
                'job' => $schedule_interview['adminJobs']['jobtitle'],
                'candidate' => "<a href=" . url('admin/candidate/detail/') . "/" . $schedule_interview['adminCandidates']['id'] . ">"
                    . $schedule_interview['adminCandidates']['name'] . "</a>",
                'date' => $s_date,
                'from' => $s_time,
                'to' => $e_time,
                'status' => '<img width="50px" height="50px" src="' . asset('status_icons') . '/' . $schedule_interview['adminStatus']['status_icon'] . '">',
            );
        }
...
//          $client = Client::where('recruitment_pipeline',1)->get() ;
        ...
        $client = AdminClient::whereIn('id', $where)->get();
    ...
    {
        $clientDetail = AdminClient::where('id', $request->client_id)->get();
    ...
    {
        $this->authorize('delete', AdminResume::class);
        $resume = AdminResume::find($request->resId);
        if ($resume != null) {
            if (file_exists(public_path() . '/files/' . $resume->resume)) {
                unlink(public_path() . '/files/' . $resume->resume);
            }
    ...
    {
        $pipelineJobs = AdminJob::where(['admin_client_id' => $request->data['client_id'], 'pipeline_id' => Auth::id()])->get();
    ...
    {
        $jobs = AdminJob::where(['admin_client_id' => $request['data']['client_id'], 'pipeline_id' => Auth::id()])->get();
    ...
    public function statusStore(Request $request)
    {
        ...
        if ($statusExist) {
            ...
            if ($request->hasFile('status_icon')) {
                $date = date('Y_m_d-H-i-s');
                $fileName = $date . '.' . $request->file('status_icon')->getClientOriginalName();
                $request->file('status_icon')->move(public_path('/status_icons/'), $fileName);
                $status->status_icon = $fileName;
...
//                $status->status_icon = $icon;
    ...
    public function statusUpdateAjax(Request $request)
    {
        ...
        if ($statusExist) {
            ...
            $status->name = $request->name;
            if ($request->hasfile('status_icon')) {
                if (file_exists(public_path() . '/status_icons/' . $status->status_icon)) {
                    unlink(public_path() . '/status_icons/' . $status->status_icon);
                } else {
                    $date = date('Y_m_d-H-i-s');
                    $fileName = $date . '.' . $request->file('status_icon')->getClientOriginalName();
                    $request->file('status_icon')->move(public_path('/status_icons/'), $fileName);
...
//                $status->status_icon = $icon;
    ...
    {
        $note = new AdminNote();
        $note->admin_candidate_id = $request['data']['candId'];
        $note->description = $request['data']['note_des'];
        $note->admin_user_id = Auth::user()->id;
        $note->save();
        // $note->id;
...
//        dd($request->all());
        ...
        if ($request->hasFile('profile_pic')) {
            if ($user->profile_pic) {
                if (file_exists(public_path() . '/assets/images/profile_pictures/' . $user->profile_pic)) {
                    unlink(public_path() . '/assets/images/profile_pictures/' . $user->profile_pic);
                }
            }

            $date = date('Y_m_d-H-i-s');
            $fileName = $date . '.' . $request->file('profile_pic')->getClientOriginalName();
            $request->file('profile_pic')->move(public_path('/assets/images/profile_pictures/'), $fileName);
    ...
    {
//    $request->all();
        $jobs = AdminJob::where(['admin_client_id' => $request->dataId])->get();
        return $jobs;
    }
...
//        dd($request->all());
    ...
    {
        $pipelineJobs = AdminJob::where(['admin_client_id' => $request->client_id, 'pipeline_id' => Auth::id()])->get();
//        dd($pipelineJobs);
        return json_encode($pipelineJobs);
    }
    ...
    public function dashSearch(Request $request)
    {
        ...
        if ($request->comName != null) {
            $client = AdminClient::with('adminNotes')
                ->whereIn('id', $where)
                ->where([['company_name', 'like', '%' . $request->comName . '%']])
                ->offset($offset)
                ->limit(10)
                ->get();
            $total = AdminClient::whereIn('id', $where)
                ->where([['company_name', 'like', '%' . $request->comName . '%']])
                ->count();
        } else {
            $client = AdminClient::with('adminNotes')
    ...
    {
        $jobDetail = AdminJob::where('id', $jobId)->get();
        $companyName = AdminClient::where('id', $jobDetail[0]['admin_client_id'])->pluck('company_name');
    ...
    public function dashboard($clientID = null)
    {
        ...
        if ($clientID == null) {
            ...
            return view('admin.pages.dashboard')
                ->with(['clients' => $client, 'jobs' => $clientDetail]);
        } else {
            $clientDetail = AdminJob::with('adminClient')
                ->where('id', $clientID)
                ->get();
    ...
    public function mainSearch()
    {
        ...
        return view('admin.search.search')
            ->with(['statuses' => AdminStatus::orderby('name', 'asc')->get(), 'clients' => $clients]);
    }
    ...
    public function searchSubmit(Request $request)
    {
        ...
        if ($where != '') {
            ...
            $candiadtes = DB::table('admin_candidates')
                ->whereRaw($where)
                ->offset($start)
                ->limit($request->length)
                ->orderBy('name', 'asc')
    ...
    public function createdInterviewStage(Request $request)
    {
        ...
        $exist = AdminStatus::where('name', $request->statusName)->exists();
//            ->orWhere('color' , $request->statusColor)->exists() ;
    ...
    {
        $interviewStages = AdminInterviewStage::all();
    ...
    public function updateInterviewStage(Request $request)
    {
        ...
        $interviewStage = AdminInterviewStage::find($request->id);

        $statusExist = AdminInterviewStage::where('name', $request->statusText)
            ->Where('id', '!=', $request->id)->exists();
...
//            return  json_encode($boolean);
//        }else{
        AdminInterviewStage::where('id', $request->statusId)->delete();
        //}
    }
    ...
    {
        $exist = AdminInterviewStatus::where('name', $request->statusName)->exists();
//            ->orWhere('color' , $request->statusColor)->exists() ;
    ...
    public function updateInterviewStatus(Request $request)
    {
        ...
        $interviewStatus = AdminInterviewStatus::find($request->id);
...
//        dd($request->all());
        if ($request->moduleId == 1) {
            $jobs = AdminJob::with('adminCandidates')
                ->where(['admin_client_id' => $request->opId, 'pipeline_id' => Auth::id()])->get();
        } else {
            $jobs = AdminJob::with('adminCandidates')
                ->where([['jobtitle', 'like', $request->opId], ['pipeline_id', Auth::id()]])->get();
        }
    ...
    public function clientDashboardJobCandidate(Request $request)
    {
        $datas = DB::table('admin_candidate_job')
            ->join('admin_jobs', 'admin_jobs.id', '=', 'admin_candidate_job.admin_job_id')
            ->join('admin_candidates', 'admin_candidates.id', '=', 'admin_candidate_job.admin_candidate_id')
            ->join('admin_statuses', 'admin_candidates.admin_status_id', '=', 'admin_statuses.id')
            ->where('admin_jobs.id', $request->jobId)
            ->select('*', 'admin_jobs.city AS job_city', 'admin_jobs.state AS job_state', 'admin_jobs.industry AS job_industry', 'admin_jobs.id AS job_id', 'admin_candidates.id AS candidate_id', 'admin_candidates.name AS candidate_name')
            ->groupBy('admin_candidates.id')->get();
        return json_encode($datas);
    }

    public function clientDashboardJobCandidateDetail(Request $request)
    {
        $candidate = AdminCandidate::with('adminStatus', 'adminNotes')
            ->where('id', $request->candId)->get();
...
//    end client dashboard
    ...
    //Front Operations
    // Front Clients Operations
    public function frontClients()
    {
        $states = State::all();
        return view('admin.front.clients.clients', compact('states'));
    }
    public function frontClientsAjaxAll(Request $request)
    {
        if ($request->current == 1) {
            $start = 0;
        } else {
        ...
        $front_clients_query = NewClient::where(function ($query) use ($request) {
            if ($request->name) {
                $query->where('company_name', 'like', '%' . $request->name . '%');
            }
            if ($request->location) {
                $query->where('complete_address', 'like', '%' . $request->location . '%');
            }
        })->where('name','!=','');

        $total_clients = $front_clients_query->count();
        $front_clients = $front_clients_query
            ->withCount('user_jobs')
            ->orderBy('user_jobs_count', 'desc')
            ->offset($start)
            ->limit($request->length)
            ->get();

        if ($front_clients) {
            $f = $front_clients->load(['user_jobs', 'industry', 'state']);
        }
        $arrayName = array('0' => $total_clients, '1' => $f);
        return json_encode($arrayName);
    }
    public function frontClientDetailsAjax(Request $request)
    {
        $front_client = NewClient::with('industry', 'state')->find($request->dataId);
        return json_encode($front_client);
    }
    public function getFrontClientDetails($id)
    {

        $industry = NewClient::find($id)->industry;
        $client = NewClient::with('user_jobs')
            ->where('id',$id)
            ->get();
        return view('admin.front.clients.client_details',compact('industry','client'));

    }

    //Front Jobs Operations
    public function frontPendingJobs()
    {
        $industries = Industry::all();
        return view('admin.front.jobs.pending_jobs', compact('industries'));
    }

    public function frontPendingJobsAjaxAll(Request $request)
    {
        if ($request->current == 1) {
            $start = 0;
        } else {
        ...
        $f = '';
        $front_jobs_query = user_job::where(function ($query) use ($request) {
            if ($request->title) {
            ...
            ->whereDate('applied_before', '>', now()->format('Y-m-d'))
            ->where('job_approved', 0);

        $totalPendingJobs = $front_jobs_query->count();
        $front_jobs = $front_jobs_query
            ->offset($start)
            ->limit($request->length)
            ->orderBy('id', 'desc')
            ->get();
        ...
        if ($front_jobs) {
            $f = $front_jobs->load(['client', 'industry']);
        }
        $arrayName = array('0' => $totalPendingJobs, '1' => $f);
        return json_encode($arrayName);
    }

    public function frontActiveJobs()
    {
        $industries = Industry::all();
        return view('admin.front.jobs.active_jobs', compact('industries'));
    }

    public function frontActiveJobsAjaxAll(Request $request)
    {
        if ($request->current == 1) {
            $start = 0;
        } else {
        ...
        $front_jobs = $front_jobs_query
            ...
            ->orderBy('id', 'desc')
            ->get();
        $totalApprovedJobs = $front_jobs_query->count();
        if ($front_jobs) {
            $f = $front_jobs->load(['client', 'industry']);
        }
        $arrayName = array('0' => $totalApprovedJobs, '1' => $f);
        return json_encode($arrayName);
    }

    public function frontInActiveJobs()
    {
        $industries = Industry::all();
        return view('admin.front.jobs.inactive_jobs', compact('industries'));
    }

    public function frontInActiveJobsAjaxAll(Request $request)
    {
        if ($request->current == 1) {
            $start = 0;
        } else {
    ...
    public function frontRejectedJobs()
    {
        $industries = Industry::all();
        return view('admin.front.jobs.rejected_jobs', compact('industries'));
    }

    public function frontRejectedJobsAjaxAll(Request $request)
    {
        if ($request->current == 1) {
            $start = 0;
        } else {
        ...
        $totalRejectedJobs = $front_jobs_query->count();
        $front_jobs = $front_jobs_query
            ->offset($start)
            ->limit($request->length)
            ->orderBy('id', 'desc')
        ...
        $arrayName = array('0' => $totalRejectedJobs, '1' => $f);
        return json_encode($arrayName);
    }

    public function frontJobDetailsAjax(Request $request)
    {
        $job = user_job::with('client', 'industry')->find($request->jobId);
        return json_encode($job);
    }

    public function getFrontJobDetails($id)
    {
        ...
        if($job->job_benefits)
        {
            $job_benefits = explode(',', $job->job_benefits);
        }
        ...
        {
            $certifications = explode(',', $job->certifications);
        }
        if($job->job_approved == 1){
            $applied_candidates = Applied_Jobs::with('candidate:id,name,phone,location')
                ->where('job_id', $id)
                ->get();
        }
        return view('admin.front.jobs.job_details',
            compact('job', 'client', 'job_benefits', 'required_skills', 'certifications', 'applied_candidates'));
    }
    ...
    public function jobData($job)
    {
        return [
            ...
            "hires" => $job->job_opening,
            "applied_before" => $job->applied_before,
            "url" => $job->web_url,
            "industry" => $job->industry->name,
            "experience" => $job->required_experience,
            "education" => $job->education->name,
            "skills" => $job->required_skills,
            "certification" => $job->certifications,
            "compensation" => packageFormat($job->package, $job->package_to, $job->package_sign, $job->package_type),
            "benefits" => $job->job_benefits,
        ];
    }
    public function jobApproval(Request $request)
    {
        $job = user_job::find($request->jobId);
        if ($job) {
            $data = $this->jobData($job);
            $email = $job->client->email;
            $job->job_approved = 1;
            $job->save();
            Mail::to($email)->send(new JobApprovalMail($data));
            return "approved";
        }
        return "fail";
    }
    ...
    public function jobRejected(Request $request)
    {
        ...
        if ($job) {
            ...
            $job->job_approved = 2;
            $job->save();
            Mail::to($email)->send(new JobRejectionMail($data));
            return "rejected";
        }
        return "fail";
    }
    public function frontJobDeleteAjax(Request $request)
    {
        $job = user_job::find($request->jobId);
        ...
        if ($job->delete()) {
            $data = $this->jobData($job);
            Mail::to($job->client->email)->send(new DeleteFrontJobMail($data));
            $status = 'deleted';
        }
    ...
    //Front Candidates Operations
    public function frontCandidates()
    {
        $industries = Industry::all();
        return view('admin.front.candidates.candidates', compact('industries'));
    }

    public function frontCandidatesAjaxAll(Request $request)
    {
        if ($request->current == 1) {
            $start = 0;
        } else {
        ...
        $f = '';
        $front_candidates_query = NewCandidate::where(function ($query) use ($request) {
            if ($request->name) {
        ...
        $front_candidates = $front_candidates_query
            ->offset($start)
            ->limit($request->length)
            ->orderBy('id', 'desc')
            ->get();

        $totalCandidates = $front_candidates_query->count();
        if ($front_candidates) {
            $f = $front_candidates->load(['industry']);
        }
        $arrayName = array('0' => $totalCandidates, '1' => $f);
        return json_encode($arrayName);
    }

    public function frontCandidateDetailsAjax(Request $request)
    {
        $candidate = NewCandidate::with('education', 'industry')->find($request->candidateId);
        return json_encode($candidate);
    }

    public function getFrontCandidateDetails($id)
    {
        ...
        {
            $job_titles = explode(',', $candidate->job_title);
        }
        if($candidate->skills)
        {
        ...
        $resume = '';
        $candidate_jobs = Applied_Jobs::with('job')
            ->where('candidate_id', $id)
            ->get();
//        if(count($candidate_jobs) > 0){
//            $resume = candidate_resume::where('id', $candidate_jobs->pluck('resume_id')[0])->first();
//        } else{
            $resume = $candidate_resume = candidate_resume::where([['candidate_id', $id], ['status', 1]])->first();
//        }
        return view('admin.front.candidates.candidate_details',
            compact('candidate', 'education', 'skills', 'certifications',
                'interests', 'candidate_jobs','job_titles', 'resume'));
    }
}
