<script>
    ...
    jQuery('#package').addClass('number-separator');
    jQuery('#package_to').addClass('number-separator');
    ...
    jQuery.validator.addMethod("validUrl", function (value, element) {
        return this.optional(element) || /^(http:\/\/www\.|https:\/\/www\.|www\.|http:\/\/|https:\/\/|http:|https:)?[a-z0-9]+([\-\.]{1}[a-z0-9]+)*\.[a-z]{1,5}(:[0-9]{1,5})?(\/.*)?$/i.test(value);
    }, "Please enter valid url.");
    ...
    jQuery.validator.addMethod("greaterThanPackage",
        ...
        var desired_pay = $(params).val().replace(/,/g, "");
        var add_range = value.replace(/,/g, "");
            if (value === "" || (parseFloat(add_range) > parseFloat(desired_pay))) {
                return true;
            }
            return false;
        });
