...
@section('title', 'Inactive Jobs Database')
<!-- Modal -->
@section('content')
    <div class="app-main__inner">
        <div class="card-header-tab card-header">
            <div class="card-header-title font-size-lg text-capitalize font-weight-normal"><i
                    class="pe-7s-users mr-3 text-muted opacity-6" style="font-size: 35px; "> </i>User side Inactive Jobs Database
            </div>
        </div>
                                ...
                                <div class="card-header-tab card-header">
                                    <div
                                        ...
                                        <i class="pe-7s-users mr-3 text-muted opacity-6"
                                           style="font-size: 35px;"> </i>Inactive Jobs
                                    </div>
                                </div>
                                ...
                                <div class="card-header-tab card-header">
                                    <div
                                        ...
                                        <i class="pe-7s-notebook mr-3 text-muted opacity-6"
                                           style="font-size: 35px;"> </i>Inactive Job Details
                                    </div>
                                </div>
                                <div class="card-body">
                                    <div class="table-responsive">
    ...
    <script type="text/javascript">
        ...
        function inactiveJobDetails(id) {
            $('.add-note').removeClass('color-active');
            $('.add-note-' + id).addClass('color-active');
            let html = '';
            let html1 = '';
            $.ajax({
                url: "{{route('front.inactive.job.details.ajax')}}",
                type: 'post',
                data: {
                    "_token": "{{ csrf_token() }}",
                    "jobId": id,
                ...
                html1 = '<button dataId=' + json['id'] + ' class="tag btn btn-link delete-job"> Delete </button>' +
        ...
        function Fun(length) {
            ...
            $('#example-2').pagination({
                ...
                ajax: function (options, refresh, $target) {
                    let html = '';
                    $.ajax({
                        url: "{{route('front.inactive.jobs.ajax.all')}}",
                        type: 'post',
                        data: {
                        ...
                        for (let i = 0; i < myJSON.length; i++) {
                            if (i === 0) {
                                inactiveJobDetails(myJSON[i]['id']);
                                html += '<tr><td colspan="2" style="background: #fff;cursor: default;padding: 1px;">' +
                                    ...
                                    'onclick=inactiveJobDetails(' + myJSON[i]['id'] + ')>' +
                        ...
                        if (html === "") {
                            document.getElementById('htmlShow').innerHTML = '<tr><td colspan="3" style="background: #fff;cursor: default;padding: 1px;">No Inactive Job Found</td></tr>';
                            document.getElementById('showJobDetails').innerHTML = `<tr><td colspan="3" style="background: #fff;cursor: default;padding: 1px;">No Inactive Job's Detail Found</td></tr>`;
        ...
        $(document).on('click', '.delete-job', function (e) {
            ...
            swal({
                ...
                if (willDelete) {
                    $.ajax({
                        url: "{{route('front.job.delete.ajax')}}",
                        type: 'post',
                        data: {
                            "_token": "{{ csrf_token() }}",
                            "jobId": id,
