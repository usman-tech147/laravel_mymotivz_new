                ...
                <div class="col-md-12">
                    <div class="card mb-3">
                        <div class="card-body">
                            ...
                            <a @can('view', App\contract::class) href="{{route('allcontract')}}" @endcan @cannot('view', App\contract::class) href="javascript:void(0)" @endcannot class="btn btn-primary btn-shine pull-right" style="margin: 0px 0px 0px 5px;">All Contracts</a>
    ...
    <script>
        ...
        function showDetail(id)
        {
            ...
            $.ajax({
                ...
                for (let i=0 ; i<json[0]['notes'].length; i++)
                {

                    var date = new Date(json[0]['notes'][i]['created_at']);
                    ...
                    notes += '<li id="candidate_note_detail_client-'+json[0]['notes'][i]['id']+'">\n' +
                        '<time datetime="">'+newDate+'</time>\n' +
                        '<p>'+json[0]['notes'][i]['description']+'</p>\n' +
                        '<a  id="note-c-dash-del-'+json[0]['notes'][i]['id']+'" data-id="'+json[0]['notes'][i]['id']+'" href="javascript:void(0)" class="tag btn note-c-dash-del">Delete</a>\n' +
                        '</li>\n' ;
                }
                if(notes=="")
                {
                ...
                html += ' <tr>\n' +
                    ...
                    '<tr>\n' +
                    '<th>Status:</th>\n' +
                    '<td><img id="client-dash-sts-'+json[0]['id']+'" width="50px" height="50px" src="'+window.location.origin+'/status_icons/'+json[0]['status']['status_icon']+'"></td>\n' +
                    // '<td><span class="status" id="client-dash-sts-'+json[0]['id']+'" style="background-color: '+json[0]['status']['color']+';"><small>'+json[0]['status']['name']+'</small></span></td>\n' +
                    '</tr>\n' +
                    '<tr>\n' +
                    '<th>Notes:</th>\n' +
                    '<td>\n' +
                ...
                btns = '<form id="client_dashboard_interview_form_'+json[0]['id']+'" action="{{route('company-schedule-interview')}}" method="post">@csrf<input type="hidden" name="candidate_id" value="'+json[0]['id']+'"><input type="hidden" id="client_dashboard_job" name="job_id" value=""><input type="hidden" id="client_dashboard_client" name="client_id" value=""></form>\n' +
                    '<a data-id="'+id+'" data-state="'+json[0]['status']['id']+'" id="update-status-client-dashboard" class="tag" data-toggle="modal" href="#statusModal-client-dashboard">Update Status</a>\n' +
