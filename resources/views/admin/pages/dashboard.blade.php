            ...
            $.ajax({
                ...
                for (let i = 0; i < json[0]['admin_candidates'].length; i++) {
                    ...
                    html += '<tr><td colspan="3" style="background: #fff;cursor: default;padding: 1px;"></td></tr>' +
                        '<tr id="candidate-row-' + json[0]['admin_candidates'][i]['id'] + '">' +
                        '<td class="add-note-candidate" data-id="' + json[0]['admin_candidates'][i]['id'] + '">' +
                        '<div id="remove_candidate-id-' + json[0]['admin_candidates'][i]['id'] + '" class="note-box">' +
                        '<a class="note-box" href="javascript:void(0)">' + json[0]['admin_candidates'][i]['name'] +
                        '</a>' +
                        '<div id="candidatesnotes-' + json[0]['admin_candidates'][i]['id'] + '" class="note-wrap ">' +
                        '<ul id="notes-list-' + json[0]['admin_candidates'][i]['id'] + '">' + newHtml +

                        '</ul>' +
                        '</div>' +
                        '</div>' +
                        '<ul class="custom-menu custom-menu-candidate-' + json[0]['admin_candidates'][i]['id'] + '">' +
                        '<li><a href="#presentModal" id="present_modal_pipeline_candidate" data-id="' + json[0]['admin_candidates'][i]['id'] + '" data-toggle="modal" class="">Present</a></li>' +
                        '<li><a href="#" onclick="candidateRemovePipline(' + json[0]['admin_candidates'][i]['id'] + ')">Remove From Pipeline</a></li>' +
                        '</ul>' +
                        '</td>' +
                        '<td><img width="50px" height="50px" src="' + window.location.origin + '/status_icons/' + json[0]['admin_candidates'][i]['admin_status']['status_icon'] + '"></td>' +
                        '<td><a href="{{url('admin/candidate/detail/')}}/' + json[0]['admin_candidates'][i]['id'] + '" class="tag">View Details</a><a class="tag" data-toggle="modal"  data-id="' + json[0]['admin_candidates'][i]['id'] + '" id="candidate_dashboard_notes" href="#dashboard_candidate_notes_modal">Add Notes</a> <a class="tag" href="{{url('admin/candidate/edit/')}}/' + json[0]['admin_candidates'][i]['id'] + '">Edit</a><a class="tag pipeline_update_status" data-toggle="modal" data-status="' + json[0]['admin_candidates'][i]['admin_status']['id'] + '"  data-id="' + json[0]['admin_candidates'][i]['id'] + '"  href="#pipelinestatusModal">Update Status</a> <a id="hired_btn" data-id="' + json[0]['admin_candidates'][i]['id'] + '" class="tag" href="#hiredModal" data-toggle="modal">Hired</a></td>\n' +
                        '</tr>';
                }
                if (html === "") {
                ...
                ajax: function (options, refresh, $target) {
                    ...
                    $.ajax({
                        ...
                        for (let i = 0; i < json[0]['clients'].length; i++) {
                            ...
                            for (let j = 0; j < json[0]['clients'][i]['admin_notes'].length; j++) {
                                var notes = json[0]['clients'][i]['admin_notes'];
                                var date = new Date(notes[j]['created_at']);
                                var mlist = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
                                var monName = mlist[date.getMonth()];
                                var day = date.getDate();
