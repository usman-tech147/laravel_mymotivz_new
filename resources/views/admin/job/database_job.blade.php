    ...
    <div class="app-main__inner">
        <div class="card-header-tab card-header">
            <div class="card-header-title font-size-lg text-capitalize font-weight-normal"><i
                    ...
                    style="font-size: 35px; color: white !important;"> </i>Job Openings
            </div>
        </div>
                    ...
                    <div class="card mb-3">
                        <div class="card-body">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <select id="selected-options-main-search-job-openinga" name=""
                                ...
                                <div class="col-md-4">
                                    <button type="button" onclick="jobsFun(10)" class="btn btn-primary pull-left">
                                        SEARCH
                                    </button>
                                </div>
                            </div>
    ...
    <script type="text/javascript">
                            ...
                            for (var i = 0; i < json.length; i++) {
                                ...
                                if (i === 0) {
                                    html += '<tr><td colspan="3" style="background: #fff;cursor: default;padding: 1px;"></td></tr>' +
                                        '<tr class="add-note add-note-' + json[i]['id'] + ' color-active" id="job-del-' + json[i]['id'] + '" onclick=showDetails1(' + json[i]['id'] + ')>' +
                                        '<td><a class="job-arrow" href="javascript:void(0)"  data-id="' + json[i]['id'] + '" id="client-info-ajax" >' +
                                        '<h4 class="bold-green">' + json[i]['jobtitle'] + '</h4></a>' +
                                        '<small><b>' + json[i]['city'] + ", " + '  ' + json[i]['state'] + '</b></small>' +
                                        '</td><td>' + comanyName + '</td><td><div class="add-cursor">' + json[i]['industry'] + '</div></td></td></tr>';
                                } else {
                                    html += '<tr><td colspan="3" style="background: #fff;cursor: default;padding: 1px;"></td></tr>' +
                                        '<tr class="add-note add-note-' + json[i]['id'] + '" id="job-del-' + json[i]['id'] + '" onclick=showDetails1(' + json[i]['id'] + ')>' +
                                        '<td><a class="job-arrow" href="javascript:void(0)" data-id="' + json[i]['id'] + '" id="client-info-ajax" >' +
                                        '<h4 class="bold-green">' + json[i]['jobtitle'] + '</h4></a>' +
                                        '<small><b>' + json[i]['city'] + ", " + '  ' + json[i]['state'] + '</b></small>' +
                                        '</td><td>' + comanyName + '</td><td><div class="add-cursor">' + json[i]['industry'] + '</div></td></tr>';
                                }
        ...
        function showDetails1(id) {
            ...
            $.ajax({
                ...
                html = '  <tr><th>Posted Date:</th><td>' + job_date_1 + '</td></tr>' +
                    '<tr><th>Job Title:</th><td>' + json[0]['jobtitle'] + '</td></tr>';
                if(json[0]['admin_client'])
                {
                    html += '<tr> <th>Employers:</th><td id="current_company_name">' + json[0]['admin_client']['company_name'] + '</td></tr>';
                }
                else{
                    html += '<tr> <th>Employers:</th><td id="current_company_name">' + 'N/A' + '</td></tr>';
                }
                html +='<tr><th>Location:</th><td>' + json[0]['city'] + ', ' + json[0]['state'] + '</td>' +
                '<tr><th>Website:</th><td>' + webUrl + '</td></tr>' +
                '<tr> <th>Compensation:</th><td>' + json[0]['package'] + '</td> </tr>' +
                '<tr> <th>Industry:</th> <td>' + json[0]['industry'] + '</td></tr>' +
                '<tr><th>service needed:</th> <td>' + serviceEmpty + '</td></tr> ' +
                '<tr> <th style="vertical-align: top;">Job Requirements:</th>' +
                '<td> <div class="job-requirements"> <P>' + json[0]['job_discription'] + '</P></div></td></tr>';
                ...
                html1 = ' <a class="tag" href="{{url("admin/job/edit/")}}/' + json[0]['id'] + '">Edit</a>' +
                    '@can('delete', App\Models\Admin\AdminJob::class)<a dataid="' + json[0]['id'] + '" class="tag job-del" href="javascript:void(0)">Delete</a>@endcan'
                    + buttonRecruiter + ' ';
                document.getElementById('table-data-ajax').innerHTML = html;
