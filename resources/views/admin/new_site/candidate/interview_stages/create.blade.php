...
@section('content')
    ...
    <div class="app-main__inner">
        <div style="display: none;" class="exist">Stage already exist</div>
        <div class="card-header-tab card-header">
            <div class="card-header-title font-size-lg text-capitalize font-weight-normal"><i class="pe-7s-user mr-3 text-muted opacity-6" style="font-size: 35px; color: #4d9a10 !important;"> </i>Interview Stages</div>
        </div>
                    ...
                    <div class="main-card mb-3 card">
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label>Select Stage Background Color</label>
    ...
    <script>
        ...
        function status() {
            ...
            $.ajax({
                ...
                for (let i=0 ; i<json.length ; i++)
                {
                    ...
                    statusHtml+= '<tr><td colspan="3" style="background: #fff;cursor: default;padding: 1px;"></td></tr><tr id="stts-row-'+json[i]['id']+'">\n' +
                        '                                    <td>'+index+'</td>\n' +
                        '                                    <td id="spanVal-'+json[i]['id']+'" style="color: '+statusTextColor+'; background-color: '+json[i]['color']+';text-align: center;">'+json[i]['name']+'</td>\n' +
                        '                                    <td><a data-toggle="modal" data-id="'+json[i]['id']+'" class="tag status-edit" href="#updateInterviewStageModel">Edit</a> <a data-id="'+json[i]['id']+'" class="tag stts-del" href="javascript:void(0)">Delete</a>\n' +
                        '                                    </td>\n' +
                        '                                </tr>' ;
        ...
        $('#statusSubmit').click(function () {
            ...
            if(statusName != "")
            {
                $.ajax({
                    ...
                    url: "{{route('interview.stages.created')}}",
                    type: 'post',
        ...
        $(document).on('click', '.status-edit', function() {
            ...
            $('#s_name_1').val(statusText) ;
            $('#s_color_1').val(hexColor) ;

        });


        $(document).on('click', '#model-interview-update', function(e) {

            var sLetestName = $('#s_name_1').val();
            var sLetestColor = $('#s_color_1').val();
            if (sLetestName != "") {

                $.ajax({
                    url: "{{route('interview.stage.update')}}",
                    type: 'post',
        ...
        $(document).on('click', '.stts-del', function(e) {
            ...
            swal({
                ...
                title: "Are you sure",
                text: "Once deleted, you will not be able to recover this Interview stage!",
                icon: "warning",
                buttons: true,
                dangerMode: true,
            })
                .then((willDelete) => {
                    if (willDelete) {
                        $.ajax({
                            url: "{{route('interview.stage.delete')}}",
                            type: 'post',
                        ...
                        swal("Your interview stage is safe!");
                    }
