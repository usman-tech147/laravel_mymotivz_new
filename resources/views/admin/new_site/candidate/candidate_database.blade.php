...
<script>
    ...
    function showDetails1(id){
        ...
        $.ajax({
            ...
            url: "{{route('new.candidate.database.detail.ajax')}}",
            type: 'post',
            ...
            var industry = myJSON[0]['industry'];
            var interest= myJSON[0]['interest'];

            if (industry.includes(',')) {
                industry = myJSON[0]['Industry'].split(',') ;
            }
            ...
            alert(myJSON[2]);
            for(let b=0 ; b<myJSON[2].length; b++)
            {
                exs = myJSON[2][b]['resume'].split('.').pop() ;
                if(exs === 'pdf')
                {
                    resume += '<a @can('download', App\Resume::class) target="_blank" href="'+window.location.origin+'/public/files/'+myJSON[2][b]['resume']+'"@endcan @cannot('download', App\Resume::class) href="javascript:void(0)" @endcannot style="font-size: 45px; margin: 0px 10px 0px 0px;" data-toggle="tooltip" title="" class="fa fa-file-pdf-o" data-original-title="'+exs+' FILE"></a>'
                }
        ...
        $('#example-3').pagination({
            ...
            ajax: function(options, refresh, $target){
                ...
                $.ajax({
                    ...
                    url: "{{route('new.candidate.database.ajax')}}",
                    type: 'post',
