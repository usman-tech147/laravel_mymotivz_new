...
@section('title', 'OTM Candidates')
@section('content')

    <div class="app-main__inner">
    <div class="card-header-tab card-header">
        <div class="card-header-title font-size-lg text-capitalize font-weight-normal"><i class="pe-7s-file mr-3 text-muted opacity-6" style="font-size: 35px; color: white !important;"> </i>OTM Candidates</div>
    </div>
...
'                               <li id="pipeline-add-otm"><a data-id="'+json[i]['id']+'" href="#pipelineModal_letest" data-toggle="modal">Add to recruitment pipeline</a></li>\n' +
'                               </ul>\n' +
'                               </td>\n' +
'                               <td>'+json[i]['city']+'</td>\n' +
'                               <td>'+json[i]['state']+'</td>\n' +
...
'                               <td ><img width="50px" height="50px" src="'+window.location.origin+'/status_icons/'+json[i]['admin_status']['status_icon']+'"></td>\n' +
...
'                               <td>'+newDate+'</td>\n' +
    ...
    //get candidate id which going to add in pipeline.
    $(document).on('click', '#pipeline-add-otm', function(e) {
        var otm_candidate = $(this).children().attr('data-id') ;
        $('#candiate-id-for-pipeline').val(otm_candidate) ;
    });
    //end get candidate id which going to add in pipeline.
